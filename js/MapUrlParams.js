    ...
    var MapUrlParams = declare([], {
      ...
      _createExtent: function() {
        ...
        if (extentArray.length === 4 || extentArray.length === 5) {
          ...
          if (!isNaN(minx) && !isNaN(miny) && !isNaN(maxx) && !isNaN(
              maxy)) {
            var wkid = 4326;
      ...
      _createMarker: function() {
        ...
        */
        require(["esri/symbols/PictureMarkerSymbol", "esri/graphic",
          "esri/dijit/PopupTemplate"
        ], lang.hitch(this, function(PictureMarkerSymbol, Graphic,
          PopupTemplate) {
          ...
          if (markerArray.length >= 2 &&
            ...
            var x = parseFloat(markerArray[0]),
              ...
              icon_url = markerArray[4] || this.defaultMarkerSymbol ||
                null,
              label = markerArray[5] || null;


            var markerSymbol = new PictureMarkerSymbol(icon_url,
              this.defaultMarkerSymbolWidth || 26, this.defaultMarkerSymbolHeight ||
              26);
            var point = new Point({
              "x": x,
              "y": y,
            ...
            this._project(point).then(lang.hitch(this, function(
              pPoint) {
              var projectedGraphic = new Graphic(pPoint,
                markerSymbol, null, infoTemplate);
      ...
      _project: function(geometry) {
        ...
        if (this._sameSpatialReference(geometry.spatialReference, this.mapSpatialReference)) {
          ...
          esriConfig.defaults.geometryService.project([geometry], this.mapSpatialReference)
            .then(lang.hitch(this, function(projectedGeometry) {
              if (projectedGeometry && projectedGeometry.length &&
                projectedGeometry.length > 0) {
      ...
      _sameSpatialReference: function(sp1, sp2) {
        ...
        if (sp1 && sp2 && sp1.wkt === sp2.wkt &&
          (sp1.wkid === sp2.wkid ||
          (esriLang.isDefined(sp1.latestWkid) && sp1.latestWkid ===
          sp2.wkid) ||
          (esriLang.isDefined(sp2.latestWkid) && sp1.wkid === sp2.latestWkid) ||
          (esriLang.isDefined(sp1.latestWkid) && sp1.latestWkid ===
          sp2.latestWkid))) {
          same = true;
        } else if (sp1 && sp2 && sp1.wkid && sp2.wkid && (array.indexOf(
            mercator, sp1.wkid) != -1) && (array.indexOf(mercator, sp2.wkid) !=
          -1)) {
          same = true;
        }
        return same;
